...
class MainWindow:
    ...
    def __init__(self):
        ...
        self._green_led_state = False
        self._green_led_count = 0
        self._green_led_blink_timer = QTimer()
        self._green_led_styles = {True: 'background-color: rgb(85, 255, 127);',
                                  False: 'background-color: rgb(0, 60, 0);'}
    ...
    def on_btn_off_released(self):
        ...
        self._off_time_pressed = None
        logging.info(f"OFF button released at {str(released_time)} with duration {str(off_pressed_duration)}")
        # Comportement en fonction du temps d'appui du bouton
        if int(off_pressed_duration) > 5:
            self._red_led_count = 6
            self._red_led_blink_timer.start(300)
            self.lever_trigger()

    def on_serial_parameter_change(self):
        self.serial_settings['PORT'] = self.ui.lineEdit_com_port.text()
