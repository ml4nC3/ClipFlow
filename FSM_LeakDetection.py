...
class LeakDetection:
    ...
    def on_state_detecting(self, current_flow):
        if self._null_flow or not self._stable_flow:
            ...
            self._limit_vol = int(self._slope * current_flow * 1000 + self._y_intercept)
            if self._limit_vol > self._config['VOL_HIGH'] * 1000:
                self._limit_vol = self._config['VOL_HIGH'] * 1000

            # Explicitation des conditions de déclenchement de l'alarme
            vol_limit_exceeded = self._current_volume >= self._limit_vol
            flow_max_exceeded = current_flow >= self._config['FLOW_MAX']

            # Si l'un des 2 tests ci-dessus est vrai : déclenchement du mode alarme
            if vol_limit_exceeded or flow_max_exceeded:
                logger.info(f'Leak detected with vol limit={vol_limit_exceeded} '
                            f'and flow={flow_max_exceeded}')
                return 'ALARM'
